...
!
!    C S McCamy, H Marcus and J G Davidson,
...
!*******************************************************************************
!
!! D_CUBERT returns the cube root of a real number.
!
!  Discussion:
!
!    D_CUBERT is designed to avoid the possible problems that can occur
...
!
!    Output, real ( kind = 8 ) D_CUBERT, the cube root of X.
!
  implicit none

  real ( kind = 8 ) d_cubert
  real ( kind = 8 ) x

  if ( 0.0D+00 < x ) then
    d_cubert = x**(1.0D+00/3.0D+00)
  else if ( x == 0.0D+00 ) then
    d_cubert = 0.0D+00
  else
    d_cubert = - ( abs ( x ) )**(1.0D+00/3.0D+00)
  end if
...
!
!    Output, real ( kind = 8 ) D_MODP, the nonnegative remainder when
!    X is divided by Y.
!
  implicit none
...
subroutine dmat_solve ( a, n, nrhs, info )
...
!
  ...
  iseed = int ( dble ( huge ( iseed ) ) * temp )
!
...
!  Parameters:
!
!    Input, integer N, the number of levels in the gray scale.
!
...
!
  ...
  if ( 3.0D+00 * h2 < 1.0D+00 ) then
    ...
    h2 = 2.0D+00 * d_pi ( ) * h2
    b = ( 1.0D+00 - s ) / 3.0D+00
    r = 1.0D+00 + s * cos ( h2 ) / cos ( d_pi ( ) / 3.0D+00 - h2 )
    r = r / 3.0D+00
    g = 1.0D+00 - b - r

  else if ( 3.0D+00 * h2 < 2.0D+00 ) then

    h2 = 2.0D+00 * d_pi ( ) * ( h2 - 1.0D+00 / 3.0D+00 )
    r = ( 1.0D+00 - s ) / 3
    g = 1.0D+00 + s * cos ( h2 ) / cos ( d_pi ( ) / 3.0D+00 - h2 )
    g = g / 3.0D+00
    b = 1.0D+00 - r - g

  else

    h2 = 2.0D+00 * d_pi() * ( h2 - 2.0D+00 / 3.0D+00 )
    g = ( 1.0D+00 - s ) / 3.0D+00
    b = 1.0D+00 + s * cos ( h2 ) / cos ( d_pi ( ) / 3.0D+00 - h2 ) 
    b = b / 3.0D+00
    r = 1.0D+00 - g - b
...
!*******************************************************************************
!
!! I_LOG_2 returns the integer part of the logarithm base 2 of |I|.
!
!  Example:
!
!     I  I_LOG_2
...
!    For positive I_LOG_2(I), it should be true that
!      2**I_LOG_2(X) <= |I| < 2**(I_LOG_2(I)+1).
!    The special case of I_LOG_2(0) returns -HUGE().
!
  implicit none
...
subroutine i_to_angle ( i, angle )

!*******************************************************************************
!
!! I_TO_ANGLE maps integers to points on a circle.
!
!  Discussion:
!
...
!  Parameters:
!
!    Input, integer NDAT, the number of data items.
!
...
!    However, these will be returned as ( -1.0, -1.0, -1.0D+00 ) if the 
...
subroutine rgb_named_uniform ( r, g, b, name )
...
!
  ...
  color_index = i_uniform ( 1, color_num, seed )
...
!  Parameters:
!
!    Input, integer SR, SG, SB, the sRGB color coordinates.
!
!    Output, real ( kind = 8 ) XCAP, YCAP, ZCAP, the corresponding CIE XYZ 
!    color coordinates.
!
...
!
  if ( ycap <= 0.0D+00 ) then
    ...
    lstar = 116.0D+00 * d_cubert ( ycap / ycapn ) - 16.0D+00
  else
    lstar = 100.0D+00
  end if
...
!    Input, real ( kind = 8 ) XCAP, YCAP, ZCAP, the CIE XYZ color coordinates.
!
!    Output, integer SR, SG, SB, the sRGB color coordinates.
!
  implicit none
