...
\title{Quadratic Bézier spline and ray intersection}
\author{Jesper Christensen, jesper@kalliope.org}

\begin{document}
\maketitle

\section{Linesegments in the plane}
...
The quadratic Bézier curve is the points traced by the function $ b: \mathbb{R} \rightarrow \mathbb{R}^2$ given control points $ p_0$, $p_1$ and $p_2$ with $p_0 = (p_{0x}, p_{0y})$, etc.
\begin{equation}
...
\end{eqnarray}
We now proceeds to find the intersection between the ray and the curve
\begin{equation}
...
\section{The swept curve in $\mathbb{R}^3$} 

Sweeping the Bézier curve a distance $e \ne 0$ along the positive z-axis yields a quadratic Bézier patch $p : \mathbb{R}^2 \rightarrow \mathbb{R}^3$, that can be parametrized as 
