...
#else
#define assertEqualColor(x, y)                                                 \
  assertTrue(IS_SORTA_EQUAL((x).r(), (y).r()) &&                               \
             IS_SORTA_EQUAL((x).g(), (y).g()) &&                               \
             IS_SORTA_EQUAL((x).b(), (y).b()))
#endif
...
public:
  void run() {
    ...
    img->clear(col);
    Texture *tex =
        new SimpleTexture(img, Vector2(1, 1), Texture::INTERPOLATION_BICUBIC);
