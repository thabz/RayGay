...
void sigchild(int n) { wait3(NULL, WNOHANG, NULL); }
#endif
...
void Webserver::addAction(string url, Action *action) { actions[url] = action; }
  ...
  if (request.method == "GET" || request.method == "HEAD") {
    if (stat(path.c_str(), &statbuf) >= 0) {
      ...
      response.addHeader("Last-Modified",
                         WebUtil::formatDate(statbuf.st_mtime));
    ...
    // TODO: If no content-header is sent, return "411 Length required" as per
    // the RFC.
