...
class GenericKdTree {
	...
	void prepare(uint32_t num, const AABox& bbox, uint32_t depth, const uint32_t dest_idx);

	bool findBestSplitPlane(uint32_t size, const AABox& bbox, CostResult& result) const;
	void findBestSplitPlane(uint32_t size, const AABox& bbox, CostResult& result, int split_dim) const;
