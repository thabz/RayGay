...
GenericKdTree<ObjectType>::GenericKdTree(uint32_t max_depth,
                                         uint32_t max_objs) {
...
template <class ObjectType>
AABox GenericKdTree<ObjectType>::enclosure(BoundedObject<ObjectType> **bobs,
                                           uint32_t num) const {
  assert(num > 0);
...
template <class ObjectType>
bool GenericKdTree<ObjectType>::findBestSplitPlane(uint32_t num,
                                                   const AABox &bbox,
...
template <class ObjectType> void GenericKdTree<ObjectType>::prepare() {
  if (prepared)
    throw_exception("Already prepared.");
  ...
  // cout << "Waste: " << (nodes.capacity() -
  // nodes.size())*sizeof(KdNode<ObjectType>) << endl;
 ...
 */
template <class ObjectType>
void GenericKdTree<ObjectType>::prepare(uint32_t num, const AABox &bbox,
                                        uint32_t depth,
...
template <class ObjectType>
inline void KdNode<ObjectType>::initLeafNode(uint32_t num,
                                             ObjectType **objects) {
  this->num = (num << 2) | 3;
  if (num == 1) {
...
template <class ObjectType>
inline void KdNode<ObjectType>::initInteriorNode(uint32_t axis, double plane,
                                                 uint32_t left) {
...
template <class ObjectType> inline bool KdNode<ObjectType>::isLeafNode() const {
  return (left & 3) == 3;
}

template <class ObjectType>
inline uint32_t KdNode<ObjectType>::getLeft() const {
  return left >> 2;
}

template <class ObjectType>
inline uint32_t KdNode<ObjectType>::getAxis() const {
  return (num & 3);
}

template <class ObjectType>
inline double KdNode<ObjectType>::getSplitValue() const {
  return splitPlane;
}

template <class ObjectType>
inline uint32_t KdNode<ObjectType>::getObjectNum() const {
  return (num >> 2);
}

template <class ObjectType>
inline ObjectType *const *KdNode<ObjectType>::getObjects() const {
  if (getObjectNum() == 1) {
    return &object;
  } else {
    return objects;
...
template <class ObjectType> inline KdNode<ObjectType>::~KdNode<ObjectType>() {
