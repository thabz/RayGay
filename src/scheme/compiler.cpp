...
SchemeObject* s_make_native_procedure(Scheme* scheme, SchemeObject* bytevector, SchemeObject* originalUserProcedure) {
    assert(originalUserProcedure->type() == SchemeObject::USER_PROCEDURE);
    return SchemeObject::createCompiledProcedure(originalUserProcedure, bytevector->bytevector, bytevector->length);
}
...
void Compiler::bind(Scheme* scheme, SchemeObject* envt) {
    scheme->assign(L"$make-native-procedure"  ,2,0,0, (SchemeObject* (*)()) s_make_native_procedure, envt);
