...
void coerceNumbers(int num, SchemeStack::iterator stack,
...
// is returned.
SchemeObject::ObjectType representativeNumberType(const wchar_t *procname,
                                                  int num,
...
SchemeObject *s_plus(Scheme *scheme, int num, SchemeStack::iterator stack) {
  ...
  if (outType == SchemeObject::REAL_NUMBER) {
    ...
    std::complex<double> *args =
        (std::complex<double> *)::alloca(sizeof(std::complex<double>) * num);
    coerceNumbers(num, stack, args);
    ...
    rational_type *args =
        (rational_type *)::alloca(sizeof(rational_type) * num);
    coerceNumbers(num, stack, args);
...
SchemeObject *s_atan(Scheme *scheme, SchemeObject *y, SchemeObject *x) {
  ...
  if (x == S_UNSPECIFIED) {
    if (y->type() == SchemeObject::COMPLEX_NUMBER) {
      ...
      // Using the formula from R^5RS
      return complex2scm((std::log(1.0 + i * z) - std::log(1.0 - i * z)) /
                         (2.0 * i));
    } else {
...
// In R6RS log takes an optional base argument, which defaults to e for the
// natural logarithm. Note that log_b(x) = log_k(x) / log_k(b) for any base k
...
SchemeObject *s_expt(Scheme *scheme, SchemeObject *a, SchemeObject *b) {
  if (a->type() == SchemeObject::INTEGER_NUMBER &&
      b->type() == SchemeObject::INTEGER_NUMBER) {
  ...
  } else if (a->type() == SchemeObject::RATIONAL_NUMBER &&
             b->type() == SchemeObject::INTEGER_NUMBER) {
...
SchemeObject *s_rationalize(Scheme *scheme, SchemeObject *s_x,
                            SchemeObject *s_y) {
  ...
  while (true) {
    rational_type p3(p1.numerator() + p2.numerator(),
...
SchemeObject *s_min(Scheme *scheme, int num, SchemeStack::iterator stack) {
  assert(num > 0);
  SchemeObject::ObjectType outType =
      representativeNumberType(L"min", num, stack);
...
SchemeObject *s_max(Scheme *scheme, int num, SchemeStack::iterator stack) {
  assert(num > 0);
  SchemeObject::ObjectType outType =
      representativeNumberType(L"max", num, stack);
...
// Using that gcd is associative thus gcd(a,b,c) = gcd(a,(gcd(b,c))) =
// gcd(gcd(a,b),c).
SchemeObject *s_gcd(Scheme *scheme, int num, SchemeStack::iterator stack) {
  ...
  SchemeObject::ObjectType outType =
      representativeNumberType(L"gcd", num, stack);
  ...
  // TODO: Make a i_exact_p macro and a
  // representativeExactness("procname",num,stack) method
  if (outType == SchemeObject::INTEGER_NUMBER ||
      outType == SchemeObject::RATIONAL_NUMBER) {
    return int2scm(result);
  } else {
    return double2scm(double(result));
  }
...
// Using the property gcd(a,b) * lcm(a,b) = a * b and that lcm(a,b,c) =
...
SchemeObject *s_lcm(Scheme *scheme, int num, SchemeStack::iterator stack) {
  ...
  SchemeObject::ObjectType outType =
      representativeNumberType(L"lcm", num, stack);
...
SchemeObject *s_make_polar(Scheme *, SchemeObject *magnitude,
                           SchemeObject *angle) {
  assert_arg_real_type(L"make-polar", 1, magnitude);
  assert_arg_real_type(L"make-polar", 2, angle);
  std::complex<double> z =
      std::polar(magnitude->realValue(), angle->realValue());
  return complex2scm(z);
}

SchemeObject *s_make_rectangular(Scheme *, SchemeObject *real,
                                 SchemeObject *imag) {
...
SchemeObject *s_nan_p(Scheme *scheme, SchemeObject *n) {
  assert_arg_number_type(L"finite?", 1, n);
  if (n->type() == SchemeObject::REAL_NUMBER) {
    return bool2scm(__isnan(scm2double(n)));
  } else {
    return S_FALSE;
  }
}
...
SchemeObject *s_equal(Scheme *scheme, int num, SchemeStack::iterator stack) {
  ...
  if (outType == SchemeObject::REAL_NUMBER) {
    ...
    if (__isnan(args[0]))
      return S_FALSE;
    for (int i = 1; i < num; i++) {
      if (__isnan(args[i]) || args[i] != args[i - 1])
        return S_FALSE;
    }
    return S_TRUE;
  } else if (outType == SchemeObject::INTEGER_NUMBER) {
...
SchemeObject *s_less(Scheme *scheme, int num, SchemeStack::iterator stack) {
  ...
  if (outType == SchemeObject::REAL_NUMBER) {
    ...
    if (__isnan(args[0]))
      return S_FALSE;
    for (int i = 1; i < num; i++) {
      if (__isnan(args[i]) || args[i - 1] >= args[i])
        return S_FALSE;
    }
    return S_TRUE;
  } else if (outType == SchemeObject::INTEGER_NUMBER) {
...
SchemeObject *s_less_equal(Scheme *scheme, int num,
  ...
  SchemeObject::ObjectType outType =
      representativeNumberType(L"<=", num, stack);
...
SchemeObject *i_complex_p(SchemeObject *n) { return i_number_p(n); }
...
SchemeObject *s_real_p(Scheme *scheme, SchemeObject *n) { return i_real_p(n); }
...
SchemeObject *i_integer_p(SchemeObject *p) {
  ...
  if (p->type() == SchemeObject::INTEGER_NUMBER) {
    ...
    return !__isnan(d) && !__isinf(d) && ::modf(d, &i) == 0.0 ? S_TRUE
                                                              : S_FALSE;
...
SchemeObject *s_exact_p(Scheme *scheme, SchemeObject *n) {
  ...
  return n->type() == SchemeObject::INTEGER_NUMBER ||
                 n->type() == SchemeObject::RATIONAL_NUMBER
             ? S_TRUE
             : S_FALSE;
}
...
SchemeObject *s_inexact_2_exact(Scheme *scheme, SchemeObject *n) {
  ...
  if (n->type() == SchemeObject::REAL_NUMBER ||
      n->type() == SchemeObject::COMPLEX_NUMBER) {
...
SchemeObject *i_string_2_number(Scheme *scheme, wstring s, uint32_t radix,
                                size_t offset) {
  int sign = 1;
  if (s.size() == 0) {
    return S_FALSE;
  ...
  if (s[offset] == L'.' && radix == 10) {
    ...
    string clipped_fraction =
        fraction.substr(0, std::min(longlong_digits10, int(fraction.size())));
    errno = 0;
  ...
  if (digits.size() != 0 || fraction.size() != 0) {
    ...
    if (imag->type() == SchemeObject::COMPLEX_NUMBER) {
      SchemeObject *real =
          SchemeObject::createRealNumber(sign * (t + df) * pow(10, double(e)));
      ;
...
void LibNumbers::bind(Scheme *scheme, SchemeObject *envt) {
  ...
  scheme->assign(L"integer?", 1, 0, 0, (SchemeObject * (*)()) s_integer_p,
                 envt);
  scheme->assign(L"complex?", 1, 0, 0, (SchemeObject * (*)()) s_complex_p,
                 envt);
  scheme->assign(L"real?", 1, 0, 0, (SchemeObject * (*)()) s_real_p, envt);
  scheme->assign(L"rational?", 1, 0, 0, (SchemeObject * (*)()) s_rational_p,
                 envt);
  scheme->assign(L"exact?", 1, 0, 0, (SchemeObject * (*)()) s_exact_p, envt);
  scheme->assign(L"inexact?", 1, 0, 0, (SchemeObject * (*)()) s_inexact_p,
                 envt);
  ...
  scheme->assign(L"exact-integer-sqrt", 1, 0, 0,
                 (SchemeObject * (*)()) s_exact_integer_sqrt, envt);
  ...
  scheme->assign(L"rationalize", 2, 0, 0, (SchemeObject * (*)()) s_rationalize,
                 envt);
  scheme->assign(L"quotient", 2, 0, 0, (SchemeObject * (*)()) s_quotient, envt);
  scheme->assign(L"remainder", 2, 0, 0, (SchemeObject * (*)()) s_remainder,
                 envt);
  ...
  scheme->assign(L"numerator", 1, 0, 0, (SchemeObject * (*)()) s_numerator,
                 envt);
  scheme->assign(L"denominator", 1, 0, 0, (SchemeObject * (*)()) s_denominator,
                 envt);
  scheme->assign(L"exact->inexact", 1, 0, 0,
                 (SchemeObject * (*)()) s_exact_2_inexact, envt);
  scheme->assign(L"inexact->exact", 1, 0, 0,
                 (SchemeObject * (*)()) s_inexact_2_exact, envt);
  scheme->assign(L"inexact", 1, 0, 0, (SchemeObject * (*)()) s_exact_2_inexact,
                 envt);
  scheme->assign(L"exact", 1, 0, 0, (SchemeObject * (*)()) s_inexact_2_exact,
                 envt);
  ...
  scheme->assign(L"infinite?", 1, 0, 0, (SchemeObject * (*)()) s_infinite_p,
                 envt);
  scheme->assign(L"finite?", 1, 0, 0, (SchemeObject * (*)()) s_finite_p, envt);
  scheme->assign(L"nan?", 1, 0, 0, (SchemeObject * (*)()) s_nan_p, envt);
  scheme->assign(L"negative?", 1, 0, 0, (SchemeObject * (*)()) s_negative_p,
                 envt);
  scheme->assign(L"positive?", 1, 0, 0, (SchemeObject * (*)()) s_positive_p,
                 envt);

  // Complex numbers
  scheme->assign(L"make-rectangular", 2, 0, 0,
                 (SchemeObject * (*)()) s_make_rectangular, envt);
  scheme->assign(L"make-polar", 2, 0, 0, (SchemeObject * (*)()) s_make_polar,
                 envt);
  scheme->assign(L"real-part", 1, 0, 0, (SchemeObject * (*)()) s_real_part,
                 envt);
  scheme->assign(L"imag-part", 1, 0, 0, (SchemeObject * (*)()) s_imag_part,
                 envt);
  scheme->assign(L"magnitude", 1, 0, 0, (SchemeObject * (*)()) s_magnitude,
                 envt);
