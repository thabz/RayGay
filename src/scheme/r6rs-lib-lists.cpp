  ...
  while (i_cdr(cropped_args[0]) != S_EMPTY_LIST) {
    ...
    for (int i = 0; i < num - 1; i++) {
      ...
      if (lists_ptr == S_EMPTY_LIST) {
        throw scheme_exception(procname +
...
SchemeObject *s_partition(Scheme *scheme, SchemeObject *proc,
                          SchemeObject *list) {
...
SchemeObject *s_fold_right(Scheme *scheme, int num,
...
inline SchemeObject *member_helper(Scheme *scheme,
                                   SchemeObject *(comparator)(Scheme *,
                                                              SchemeObject *,
                                                              SchemeObject *),
...
SchemeObject *assoc_helper(Scheme *scheme,
                           SchemeObject *(comparator)(Scheme *, SchemeObject *,
                                                      SchemeObject *),
                           SchemeObject *obj, SchemeObject *alist) {
...
SchemeObject *s_cons_star(Scheme *scheme, int num,
...
void R6RSLibLists::bind(Scheme *scheme, SchemeObject *envt) {
  ...
  scheme->assign(L"partition", 2, 0, 0, (SchemeObject * (*)()) s_partition,
                 envt);
  scheme->assign(L"fold-left", 3, 0, 1, (SchemeObject * (*)()) s_fold_left,
                 envt);
  scheme->assign(L"fold-right", 3, 0, 1, (SchemeObject * (*)()) s_fold_right,
                 envt);
