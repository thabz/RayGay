...
inline int64_t SchemeObject::integerValue() const {
  ObjectType t = type();
  if (t == INTEGER_NUMBER)
...
inline rational_type SchemeObject::rationalValue() const {
  ObjectType t = type();
...
inline double SchemeObject::realValue() const {
  ObjectType t = type();
  if (t == REAL_NUMBER)
    return real_value;
...
inline std::complex<double> SchemeObject::complexValue() const {
  ObjectType t = type();
  if (t == COMPLEX_NUMBER) {
