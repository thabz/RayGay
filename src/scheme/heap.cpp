...
void Heap::addRoot(SchemeObject *root) { roots.push_back(root); }

void Heap::popRoot() { roots.pop_back(); }
...
void Heap::garbageCollect(vector<SchemeObject *> &stack) {
  pthread_mutex_lock(&mutex_reserve);
  // cout << "BEFORE: Size of heap: " << slots_num << " free: " << free_slots <<
  // " (" << banks.size() << ")" << endl;
  // cout << "BEFORE: Size of roots: " << roots.size() << endl;
  mark(stack);
  sweep();
  // cout << "AFTER: Size of heap: " << slots_num << " free: " << free_slots <<
  // endl; cout << "AFTER: Size of roots: " << roots.size() << endl << endl;
  allocated = 0;
  gc_runs++;
  pthread_mutex_unlock(&mutex_reserve);
}
...
void Heap::mark(vector<SchemeObject *> &stack) {
  ...
  */
  for (vector<SchemeObject *>::iterator i = stack.begin(); i != stack.end();
       i++) {
    assert(*i != NULL);
    (*i)->mark();
  }
  for (vector<SchemeObject *>::iterator i = roots.begin(); i != roots.end();
       i++) {
    assert(*i != NULL);
    (*i)->mark();
  }
  ...
  for (uint32_t i = 0; banks_iterator != banks.end(); i++, banks_iterator++) {
    ...
    for (uint32_t j = 0; j < page_size; j++, cur++) {
      if (cur->type() != SchemeObject::BLANK &&
          cur->type() != SchemeObject::RESERVED) {
...
void Heap::dumpStats() {
  ...
  for (int i = 0; i < SchemeObject::ALL_TYPE_ARE_BEFORE_HERE; i++) {
    ...
    if (count > 0) {
      ...
      wcout << "    " << left << setw(20) << (type_name + L"s") << ": " << count
            << endl;
    }
  }
