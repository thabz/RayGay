...
SchemeObject *Syntax::transcribe(SchemeObject *transformer,
                                 SchemeObject *expr) {
 ...
 */
SchemeObject *Syntax::find_template(SchemeObject *transformer,
                                    SchemeObject *expr) {
    ...
    if (s_member(P, ellipsis_symbol) == S_TRUE) {
      ...
      if (i_pair_p(i_cdr(P)) == S_TRUE && i_cadr(P) == ellipsis_symbol) {
        ...
        while (F != S_EMPTY_LIST) {
          if (!matches(literals, Pn1, i_car(F)))
            return false;
          F = i_cdr(F);
        }
        return true;
      } else {
        return i_pair_p(F) == S_TRUE && matches(literals, i_car(P), i_car(F)) &&
