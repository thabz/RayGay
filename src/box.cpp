...
bool Box::onEdge(const Vector &p) const {
...
bool Box::checkIntersect(const Ray& ray) const {
    Vector B = ray.origin;
    Vector v_inv = ray.inv_direction;
    Vector v = ray.direction;
...
Box Box::doUnion(const Box& b1, const Box& b2) {
...
void Box::test() {
    Box b;
    /* Test normal() */
    b = Box(Vector(-1,-1,-1),Vector(1,1,1));
    ...
    b = Box(Vector(-100,-10,-100),Vector(100,10,100));
    ...
    /* Test doUnion */
    Box b1 = Box(Vector(-1,-1,-1),Vector(1,1,1));
    Box b2 = Box(Vector(-2,-2,-2),Vector(0,0,0));
    Box bu = Box::doUnion(b1,b2);
    assert(Box(Vector(-2,-2,-2),Vector(1,1,1)) == bu);

    Box bi = Box::doIntersection(b1,b2);
    assert(Box(Vector(-1,-1,-1),Vector(0,0,0)) == bi);
