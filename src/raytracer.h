...
public:
  /// Default constructor
  Raytracer(RendererSettings *settings, Image *img, Scene *scene, KdTree *spc,
...
private:
  ...
  RGBA traceSub(const bool intersected, Intersection &i, const Ray &,
                const int depth);
  RGBA tracePrimary(const Ray &);
  RGB calculate_reflection(const Ray &ray, const Intersection &intersection,
...
inline RGBA Raytracer::tracePrimary(const Ray &ray) {
  if (ray.ignore()) {
    return RGBA(0, 0, 0, 0);
  }
...
inline RGBA Raytracer::trace(const Ray &ray, const int depth) {
