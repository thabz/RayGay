...
Tessalation::Tessalation(const Vector center, const double radius,
                         const uint32_t num, const Material *mat)
...
void Tessalation::split(const Vector &v1, const Vector &v2, const Vector &v3,
                        uint32_t depth) {
  if (depth > max_splits) {
    Mesh::addTriangle(findOrAddVertex(v1), findOrAddVertex(v2),
                      findOrAddVertex(v3));
  } else {
