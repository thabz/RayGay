...
/**
 * Constructs a torus symmetric about the y-axis, ie. placed in zx-plane
 centered in origin.
 *
...
void Torus::transform(const Matrix &m) { Transformer::transform(m); }

void Torus::_fullIntersect(const Ray &world_ray, const double t,
...
double Torus::_fastIntersect(const Ray &world_ray) const {
  ...
  double root;
  int num =
      Math::solveQuarticSingle(a1, a2, a3, a4, 2 * EPSILON - closer, &root);
...
SceneObject *Torus::clone() const { return new Torus(*this); }

uint32_t Torus::maxIntersections() const { return 4; }
...
double Torus::signedDistance(const Vector &point) const {
  Vector p = pointToObject(point);
  Vector2 pxz = Vector2(p.x(), p.z());
  Vector2 q = Vector2(pxz.length() - R, p.y());
  return q.length() - r;
}
