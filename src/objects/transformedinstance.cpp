...
TransformedInstance::TransformedInstance(Object *object)
    : Object(object->getMaterial()) {
  this->object = object;
}

TransformedInstance::TransformedInstance(Object *object, Material *material)
    : Object(material) {
  this->object = object;
}

void TransformedInstance::_fullIntersect(const Ray &ray, const double t,
...
uint32_t TransformedInstance::allIntersections(const Ray &ray,
                                               Intersection *result) const {
  Solid *solid = dynamic_cast<Solid *>(object);
  if (solid == NULL) {
    throw_exception("Transformed instance is not a solid");
