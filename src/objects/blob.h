...
public:
  ...
  virtual int intersects(const AABox &voxel_bbox,
                         const AABox &obj_bbox) const = 0;
...
public:
  BlobAtomCylinder(double radius, double weight, const Vector &from,
                   const Vector &to);
  AABox getBoundingBox() const;
  double squaredDistToPoint(const Vector &point) const;
  int intersects(const AABox &voxel_bbox, const AABox &obj_bbox) const;
 ...
 *
 * We use the field function attributed to the Wyvill bros. for their Soft
 * Objects.
 *
 * \f[
 * P_i = \left\{
 * \begin{array}{cc}
 *   a_i(1 - \frac{4}{9}\frac{r_i^6}{R_i^6} + \frac{17}{9}\frac{r_i^4}{R_i^4} -
 * \frac{22}{9}\frac{r_i^2}{R_i^2}) & r_i \leq R_i \\ 0 & r_i > R_i \end{array}
 * \right.
 * \f]
 *
 * where \f$ r_i \f$ is the distance from \f$(x,y,z)\f$ to the center of \f$ S_i
 * \f$, \f$ R_i \f$ is its radius of influence and \f$ a_i \f$ is it's weight.
 *
...
public:
  ...
  void addAtom(const Vector &from, const Vector &to, double radius,
               double weight);
  void prepare();
