...
public:
  ExtrudedCurve(const Vector2 &p0, const Vector2 &p1, const Vector2 &p2,
...
public:
  GlyphFace(TrueTypeFont::Glyph *glyph, double z_direction,
            const Material *material);
  AABox getBoundingBox() const;
  void transform(const Matrix &m);
  SceneObject *clone() const;
...
public:
  Text(std::wstring text, TrueTypeFont *font, double size, double depth,
       const Material *material);
};
