...
Julia::Julia(Quaternion c, uint32_t max_iter, uint32_t steps, double accuracy,
             double w_axis_offset, Material *mat)
    : IsoSurface(steps, accuracy, 0, mat) {
  this->c = c;
...
Vector Julia::normal(const Vector &point) const {
  ...
  Vector normal = Vector(x2_delta.norm() - x1_delta.norm(),
...
AABox Julia::_getBoundingBox() const {
  return AABox(Vector(-max_d, -max_d, -max_d), Vector(max_d, max_d, max_d));
}

SceneObject *Julia::clone() const { return new Julia(*this); }
