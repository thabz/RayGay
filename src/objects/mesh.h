...
public:
  ...
  /// The types of meshes
  enum MeshType { MESH_FLAT, MESH_PHONG };
  ...
  void addConvexPolygon(int num, const uint32_t *c, const uint32_t *uv = NULL,
                        const uint32_t *n = NULL);

  /// Add a quad to the mesh
  void addQuad(const uint32_t c[4], const uint32_t uv[4] = NULL,
               const uint32_t n[4] = NULL);
  ...
  void addTriangle(const uint32_t v[3], const uint32_t uv[3] = NULL,
                   const uint32_t n[3] = NULL);

  /// Add a triangle by vertex indices
  void addTriangle(uint32_t v0, uint32_t v1, uint32_t v2, uint32_t uv0,
...
inline uint32_t Mesh::facesNum() const { return faces.size() / 3; }

inline Vector Mesh::cornerAt(uint32_t tri_idx, uint32_t i) const {
  return corners[faces[3 * tri_idx + i]];
}

inline void Mesh::cornerAt(uint32_t tri_idx, uint32_t i, double dest[3]) const {
