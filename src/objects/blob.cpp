...
BlobAtomSphere::BlobAtomSphere(double radius, double weight,
                               const Vector &center)
    : BlobAtom(radius, weight) {
  this->center = center;
}
...
inline double BlobAtomSphere::squaredDistToPoint(const Vector &point) const {
 ...
 * MA, 1990.
 */
int BlobAtomSphere::intersects(const AABox &voxel_bbox,
...
BlobAtomCylinder::BlobAtomCylinder(double radius, double weight,
...
int BlobAtomCylinder::intersects(const AABox &voxel_bbox,
                                 const AABox &obj_bbox) const {
  return 0;
}
...
BlobTree::BlobTree()
    : GenericKdTree<BlobAtom>(KD_TREE_MAX_DEPTH, KD_TREE_MAX) {}
 ...
 */
Blob::Blob(double iso, uint32_t steps, double accuracy, Material *material)
    : IsoSurface(steps, accuracy, iso, material) {
  tree = new BlobTree();
  num = 0;
}

Blob::~Blob() { delete tree; }
 ...
 * @param weight weight to use
 */
void Blob::addAtom(const Vector &from, const Vector &to, double radius,
                   double weight) {
  if (from == to) {
...
AABox Blob::_getBoundingBox() const { return tree->boundingBox(); }
