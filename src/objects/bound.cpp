...
void Bound::transform(const Matrix &m) { group->transform(m); }

AABox Bound::getBoundingBox() const { return tree->boundingBox(); }
...
void Bound::fullIntersect(const Ray &ray, double t,
...
void Bound::_fullIntersect(const Ray &ray, const double t,
                           Intersection &result) const {
