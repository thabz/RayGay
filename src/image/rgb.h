...
inline const double &RGB::operator[](const int i) const {
  // assert(i>=0 && i<3);
  return _vector[i];
}

inline double &RGB::operator[](const int i) {
  // assert(i>=0 && i<3);
  return _vector[i];
}

inline RGB RGB::operator*(const RGB &o) const {
  return RGB(r() * o.r(), g() * o.g(), b() * o.b());
}

inline RGB RGB::operator*(double c) const {
 ...
 * Rec. 709]
 */
inline double RGB::brightness() const {
  return 0.2125 * r() + 0.7154 * g() + 0.0721 * b();
}

inline bool RGB::operator==(const RGB &v) const {
  return _vector[0] == v[0] && _vector[1] == v[1] && _vector[2] == v[2];
}

inline RGB RGB::toGrayscale() const {
  double g = brightness();
  return RGB(g, g, g);
}

inline RGB RGB::operator+(const RGB &v) const {
  return RGB(v._vector[0] + _vector[0], v._vector[1] + _vector[1],
...
inline RGB &RGB::operator*=(const double x) {
  double *d = _vector;
  *d++ *= x;
  *d++ *= x;
  *d *= x;
...
inline double RGB::norm() const {
  return _vector[0] * _vector[0] + _vector[1] * _vector[1] +
         _vector[2] * _vector[2];
}

inline RGB operator*(const double x, const RGB &v) {
