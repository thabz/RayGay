...
Image *PngIO::load(const std::string &filename, Allocator::model_t model) {
  ...
  //   png_bytep row = (png_bytep) png_malloc(png_ptr, png_get_rowbytes(png_ptr,
  //   info_ptr));
  ...
  for (uint32_t y = 0; y < height; y++) {
    ...
    for (uint32_t x = 0; x < width; x++) {
      RGBA col;
      if (bpp == 4) {
        col = RGBA(rowp[4 * x + 0] / 255.0, rowp[4 * x + 1] / 255.0,
                   rowp[4 * x + 2] / 255.0, rowp[4 * x + 3] / 255.0);
      } else if (bpp == 3) {
        col = RGBA(rowp[3 * x + 0] / 255.0, rowp[3 * x + 1] / 255.0,
                   rowp[3 * x + 2] / 255.0, 1);
      } else if (bpp == 1) {
        col = RGBA(rowp[x] / 255.0, rowp[x] / 255.0, rowp[x] / 255.0, 1);
      }
