...
class ImageImpl : public Image {
    public:
        ...
        ImageImpl(const Image& image, Allocator::model_t alloc_model = Allocator::AUTO);
...
template<class T, int N>
inline
ImageImpl<T,N>::ImageImpl(int w, int h, Allocator::model_t alloc_model) : Image(w,h)
{
    block_size = 16;
    ...
    data = (T*) Allocator::safe_allocate(alloc_size_pixels * N * sizeof(T), alloc_model);
}

template<class T, int N>
inline
ImageImpl<T,N>::ImageImpl(const Image& other, Allocator::model_t alloc_model)
{
...
template<class T, int N>
inline
uint32_t ImageImpl<T,N>::offset(int x, int y) const 
{
    return (blockno(x,y)*block_size*block_size + (y%block_size)*block_size + (x%block_size))*N;
}

template<>
inline
RGBA ImageImpl<double,3>::getRGBA(int x, int y) const
{
...
template<>
inline
RGBA ImageImpl<double,4>::getRGBA(int x, int y) const
{
    uint32_t o = offset(x,y);
...
template<>
inline
RGBA ImageImpl<float,3>::getRGBA(int x, int y) const
{
    uint32_t o = offset(x,y);
    return RGBA(data[o+0], data[o+1], data[o+2], 1);
}
...
template<>
inline
RGBA ImageImpl<float,4>::getRGBA(int x, int y) const
{
    uint32_t o = offset(x,y);
    return RGBA(data[o+0], data[o+1], data[o+2], data[o+3]);
}
...
template<>
inline
RGBA ImageImpl<uint8_t,3>::getRGBA(int x, int y) const
{
    uint32_t o = offset(x,y);
...
template<>
inline
RGBA ImageImpl<uint8_t,4>::getRGBA(int x, int y) const
{
    uint32_t o = offset(x,y);
    return RGBA(data[o+0] / 255.0, data[o+1] / 255.0, data[o+2] / 255.0, data[o+3] / 255.0);
}
...
template<>
inline
void ImageImpl<double,4>::setRGBA(int x, int y, const RGBA& c)
{
    uint32_t o = offset(x,y);
    data[o+0] = c.r();
    data[o+1] = c.g();
...
template<>
inline
void ImageImpl<float,3>::setRGBA(int x, int y, const RGBA& c)
{
    uint32_t o = offset(x,y);
    data[o+0] = float(c.r());
    data[o+1] = float(c.g());
...
template<>
inline
void ImageImpl<float,4>::setRGBA(int x, int y, const RGBA& c)
{
    uint32_t o = offset(x,y);
    data[o+0] = float(c.r());
    data[o+1] = float(c.g());
...
template<>
inline
void ImageImpl<uint8_t,3>::setRGBA(int x, int y, const RGBA& c)
{
    uint32_t o = offset(x,y);
...
template<>
inline
void ImageImpl<uint8_t,4>::setRGBA(int x, int y, const RGBA& c)
{
    uint32_t o = offset(x,y);
