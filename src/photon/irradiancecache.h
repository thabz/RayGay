...
public:
   ...
   * @return whether a irradiance estimate with a sufficiently high weight was
   * found
   */
   ...
   * @param hmd the harmonic mean distance to the objects that was hit by final
   * gather rays.
   *
   */
  void putEstimate(const Vector &point, const Vector &normal,
                   const RGB &irradiance, const double hmd);

private:
  class CacheNode {
  public:
    CacheNode(const Vector &point, const Vector &normal, const RGB &irradiance,
              double hmd, double a);

    double getWeight(const Vector &point, const Vector &normal) const;

    RGB getIrradiance() const {
      return RGB(irradiance[0], irradiance[1], irradiance[2]);
    };
  ...
  void traverseOctree(const HierarchyNode *const node, const Vector &point,
