...
inline void Sampler::setPixel(int x, int y, const RGBA &color) {
  image->setRGBA(x, image->getHeight() - y - 1, color);
}

inline RGBA Sampler::sample(double x, double y) {
  return renderer->getPixel(
      Vector2(x / image->getWidth(), y / image->getHeight()));
}

inline RGBA Sampler::sample(const Vector2 &pos) {
