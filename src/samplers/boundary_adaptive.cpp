...
BoundaryAdaptive::BoundaryAdaptive(Image* image, Renderer* renderer, uint32_t aa_depth) : Sampler(image,renderer) 
{
 ...
 */
void BoundaryAdaptive::prepareCurRow(std::vector<PixelBlock>* cur_row, std::vector<PixelBlock>* prev_row,uint32_t blocksize) {
 ...
 */
void BoundaryAdaptive::prepareCurBlock(PixelBlock* cur_block, PixelBlock* prev_block, uint32_t blocksize) {
...
RGBA BoundaryAdaptive::getSubPixel(uint32_t curLevel, const Vector2& center, PixelBlock *block, double size, int x1, int y1, int x2, int y2) {
...
inline
void BoundaryAdaptive::PixelBlock::reset() {
...
Sampler* BoundaryAdaptiveFactory::createInstance(Image* img, Renderer* renderer)
{
