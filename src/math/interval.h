...
inline Vector2 Interval::getSegment(int i) const {
  return Vector2(segments[i * 2 + 0], segments[i * 2 + 1]);
}

inline void Interval::setSegment(Vector2 s, int i) {
  segments[i * 2 + 0] = s[0];
  segments[i * 2 + 1] = s[1];
}

inline uint32_t Interval::getSegmentsNum() const { return segments.size() / 2; }
