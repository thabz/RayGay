...
inline Vector2::Vector2() { _vector[0] = _vector[1] = 0; }
...
inline Vector2 operator*(const double x, const Vector2 &v) {
...
inline double &Vector2::operator[](const int i) {
  // assert(i>=0 && i<3);
  return _vector[i];
}

inline const double &Vector2::operator[](const int i) const {
  // assert(i>=0 && i<3);
  return _vector[i];
}
...
inline bool Vector2::operator!=(const Vector2 &x) const {
  return IS_NEQUAL(x[0], _vector[0]) || IS_NEQUAL(x[1], _vector[1]);
}

inline bool Vector2::operator()(const Vector2 *v1, const Vector2 *v2) const {
  return (*v1) == (*v2);
}
