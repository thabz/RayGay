...
void Particle::move(const Vector &v) { position += v; }

Spring::Spring(Particle *pa1, Particle *pa2, double s, double damping,
               double rest_length) {
  /// If no rest_length is supplied the current distance between the particles
  /// is used
  if (rest_length == -1) {
...
Attraction::Attraction(Particle *p1, Particle *p2, double strength,
...
void ParticleSystem::add(Particle *particle) { particles.push_back(particle); }

void ParticleSystem::add(Spring *spring) { springs.push_back(spring); }
