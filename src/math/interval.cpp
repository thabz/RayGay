...
bool Interval::isEmpty() const { return segments.size() == 0; }

double Interval::length() const {
  double length = 0.0;
  for (std::vector<double>::const_iterator p = segments.begin();
       p != segments.end(); p += 2) {
    length += (*(p + 1) - *p);
  }
  return length;
...
bool Interval::operator!=(const Interval &x) const { return !(*this == x); }

ostream &operator<<(ostream &os, const Interval &v) {
  if (v.isEmpty()) {
    os << "NIL";
  } else {
    for (std::vector<double>::const_iterator p = v.segments.begin();
         p != v.segments.end(); p += 2) {
      os << "[" << *p << "," << *(p + 1) << "] ";
    }
  }
  return os;
