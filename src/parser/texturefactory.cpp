...
SchemeObject* TextureFactory::make_multi_texture(Scheme* scheme, SchemeObject* s_filenames, SchemeObject* s_tiles_per_row, SchemeObject* s_memory_cached, SchemeObject* s_repeat_x, SchemeObject* s_repeat_y, SchemeObject* s_interpolation_type) {
    ...
    Texture* texture = new MultiTexture(filenames, tiles_per_row, memory_cached, Vector2(rep_x,rep_y), type);
    return texture2scm(texture);
}
...
void TextureFactory::register_procs(Scheme* scheme) {
    ...
    scheme->assign(L"make-multi-texture",6,0,0,(SchemeObject* (*)()) TextureFactory::make_multi_texture);
