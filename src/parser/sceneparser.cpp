...
SceneParser::SceneParser(Scene *scene) {
  ...
  scheme->assign(L"set-settings", 1, 0, 0,
                 (SchemeObject * (*)()) SceneParser::set_settings);
  scheme->assign(L"__add-to-scene__", 1, 0, 0,
                 (SchemeObject * (*)()) SceneParser::add_to_scene);
...
void SceneParser::parse_expr(std::wstring expr) { scheme->eval(expr); }
...
void SceneParser::parse_file(std::wstring filename) {
  ...
  SchemeObject *port = s_open_file_input_port(scheme, string2scm(filename),
...
SchemeObject *SceneParser::add_to_scene(Scheme *scheme, SchemeObject *s_value) {
  if (s_sceneobject_p(scheme, s_value) == S_TRUE) {
    SceneObject *sceneobject =
        scm2sceneobject(s_value, L"internal-populate-scene", 0);
    Object *o = dynamic_cast<Object *>(sceneobject);
    if (o != NULL && o->getMaterial() == NULL) {
      throw scheme_exception(L"add-to-scene",
                             L"Object added without material: " +
                                 s_value->toString());
    }
    scene->addObject(sceneobject);
  } else if (s_lightsource_p(scheme, s_value) == S_TRUE) {
    Lightsource *light =
        scm2lightsource(s_value, L"internal-populate-scene", 0);
    scene->addLight(light);
  } else {
    throw scheme_exception(L"internal-populating-scene",
                           L"A non-sceneobject or non-lightsource found: " +
                               s_value->toString());
  }
...
void SceneParser::populate(Scene *scene, RendererSettings *renderersettings) {
  ...
  if (s_renderer != S_EMPTY_LIST) {
    ...
    if (r_string == L"raytracer") {
      ...
      type = RendererSettings::NONE;
      throw scheme_exception(L"internal-setting-renderer",
                             L"Unknown renderertype: " + r_string);
    }
  } else {
    type = RendererSettings::NONE;
  }
...
SchemeObject *SceneParser::set_settings(Scheme *scheme,
                                        SchemeObject *s_settings) {
  ...
  if (S_FALSE == i_null_p(s_settings)) {
    ...
    for (uint32_t i = 0; i < argc; i++) {
      ...
      SchemeObject *s_value =
          s_list_ref(scheme, s_settings, int2scm(i * 2 + 1));
...
string SceneParser::version() { return string("RayGay Scheme ") + VERSION; }
