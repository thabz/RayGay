...
SchemeObject *make_sphere(Scheme *scheme, SchemeObject *s_center,
                          SchemeObject *s_radius, SchemeObject *s_material) {
  const wchar_t *proc = L"make-sphere";
...
SchemeObject *make_halfspace(Scheme *scheme, SchemeObject *s_obj1,
                             SchemeObject *s_obj2, SchemeObject *s_obj3,
                             SchemeObject *s_obj4) {
...
SchemeObject *make_ellipsoid(Scheme *scheme, SchemeObject *s_center,
                             SchemeObject *s_radii, SchemeObject *s_material) {
  const wchar_t *proc = L"make-ellipsoid";
  Vector center = scm2vector(s_center, proc, 1);
...
SchemeObject *make_torus(Scheme *scheme, SchemeObject *s_R, SchemeObject *s_r,
                         SchemeObject *s_material) {
  const wchar_t *proc = L"make-to   rus";
...
SchemeObject *make_box(Scheme *scheme, SchemeObject *s_corner1,
                       SchemeObject *s_corner2, SchemeObject *s_material) {
  const wchar_t *proc = L"make-box";
...
SchemeObject *make_solid_box(Scheme *scheme, SchemeObject *s_corner1,
                             SchemeObject *s_corner2,
                             SchemeObject *s_material) {
  const wchar_t *proc = L"make-solid-box";
  Vector corner1 = scm2vector(s_corner1, proc, 1);
  Vector corner2 = scm2vector(s_corner2, proc, 2);
  Material *material;
  if (s_material == S_UNSPECIFIED) {
...
SchemeObject *make_cylinder(Scheme *scheme, SchemeObject *s_begin,
                            SchemeObject *s_end, SchemeObject *s_radius,
                            SchemeObject *s_material) {
  const wchar_t *proc = L"make-cylinder";
...
SchemeObject *make_uncapped_cylinder(Scheme *scheme, SchemeObject *s_begin,
                                     SchemeObject *s_end,
                                     SchemeObject *s_radius,
                                     SchemeObject *s_material) {
  const wchar_t *proc = L"make-uncapped-cylinder";
  Vector begin = scm2vector(s_begin, proc, 1);
  Vector end = scm2vector(s_end, proc, 2);
  double radius = safe_scm2double(s_radius, 3, proc);
  Material *material;
...
SchemeObject *make_cone(Scheme *scheme, SchemeObject *s_begin,
                        SchemeObject *s_end, SchemeObject *s_radius_begin,
                        SchemeObject *s_radius_end, SchemeObject *s_material) {
  const wchar_t *proc = L"make-cone";
  Vector begin = scm2vector(s_begin, proc, 1);
  Vector end = scm2vector(s_end, proc, 2);
...
SchemeObject *make_uncapped_cone(Scheme *scheme, SchemeObject *s_begin,
                                 SchemeObject *s_end,
                                 SchemeObject *s_radius_begin,
                                 SchemeObject *s_radius_end,
                                 SchemeObject *s_material) {
  const wchar_t *proc = L"make-uncapped-cone";
  Vector begin = scm2vector(s_begin, proc, 1);
  Vector end = scm2vector(s_end, proc, 2);
  double radius_begin = safe_scm2double(s_radius_begin, 3, proc);
  double radius_end = safe_scm2double(s_radius_end, 4, proc);
 ...
 */
SchemeObject *make_extrusion(Scheme *scheme, SchemeObject *s_path,
                             SchemeObject *s_circle, SchemeObject *s_twists,
                             SchemeObject *s_segments, SchemeObject *s_pieces,
                             SchemeObject *s_material) {
  const wchar_t *proc = L"make-extrusion";
  ...
  Extrusion *extrusion =
      new Extrusion(*path, *circle, segments, pieces, twists, material);
  return sceneobject2scm(extrusion);
}

SchemeObject *make_heightfield(Scheme *scheme, SchemeObject *s_texture,
                               SchemeObject *s_box, SchemeObject *s_w_div,
                               SchemeObject *s_d_div,
                               SchemeObject *s_material) {
  const wchar_t *proc = L"make-heightfield";
  ...
  HeightField *hf = new HeightField(texture, box[1], box[0], box[2],
                                    width_divisions, depth_divisions, material);
  return sceneobject2scm(hf);
}

SchemeObject *make_blob(Scheme *scheme, SchemeObject *s_iso,
                        SchemeObject *s_steps, SchemeObject *s_accuracy,
                        SchemeObject *s_material, SchemeObject *s_atoms) {
  const wchar_t *proc = L"make-blob";
...
SchemeObject *SceneObjectFactory::make_isosurface(
    Scheme *scheme, SchemeObject *s_proc, SchemeObject *s_vec_lower,
    SchemeObject *s_vec_higher, SchemeObject *s_iso, SchemeObject *s_steps,
    SchemeObject *s_accuracy, SchemeObject *s_material) {
  const wchar_t *proc = L"make-isosurface";
...
SchemeObject *make_sdf_object(Scheme *scheme, SchemeObject *s_solid,
  ...
  double accuracy = safe_scm2double(s_accuracy, 2, proc);
  Material *material = scm2material(s_material, proc, 3);

  SDFObject *sdf_object = new SDFObject(solid, accuracy, material);

  return sceneobject2scm(sdf_object);
}

SchemeObject *SceneObjectFactory::make_parametrized_surface(
    Scheme *scheme, SchemeObject *s_proc, SchemeObject *s_u_res,
    SchemeObject *s_v_res, SchemeObject *s_u_close, SchemeObject *s_v_close,
    SchemeObject *s_material) {
  const wchar_t *proc = L"make-parametrized-surface";
  ...
  SchemeParametrizedSurface *surface = new SchemeParametrizedSurface(
      scheme, s_proc, uRes, vRes, uClose, vClose, material);
  return sceneobject2scm(surface);
}

SchemeObject *make_mesh(Scheme *scheme, SchemeObject *s_material,
                        SchemeObject *s_vertices, SchemeObject *s_triangles) {
  const wchar_t *proc = L"make-mesh";
  ...
  if (Environment::getUniqueInstance()->isVerbose()) {
    cout << "Created mesh with " << vlength << " vertices and " << flength
         << " faces." << endl;
  }
...
SchemeObject *make_ply_mesh(Scheme *scheme, SchemeObject *s_filename,
                            SchemeObject *s_material) {
  const wchar_t *proc = L"make-ply-mesh";
...
SchemeObject *make_obj_mesh(Scheme *scheme, SchemeObject *s_filename,
                            SchemeObject *s_material) {
  const wchar_t *proc = L"make-obj-mesh";
  wstring filename = scm2string(s_filename);
  Material *material = scm2material(s_material, proc, 2);
  OBJ *obj;
  try {
...
SchemeObject *make_bezierpatch(Scheme *scheme, SchemeObject *s_points,
                               SchemeObject *s_xres, SchemeObject *s_yres,
                               SchemeObject *s_material) {
  const wchar_t *proc = L"make-bezierpatch";
...
SchemeObject *make_difference(Scheme *scheme, SchemeObject *s_left,
                              SchemeObject *s_right, SchemeObject *s_material) {
  const wchar_t *proc = L"make-difference";
  Material *material;
  if (s_material == S_UNSPECIFIED) {
    material = NULL;
  } else {
  ...
  if (solid_left == NULL)
    wrong_type_arg(proc, 1, s_left);
  if (solid_right == NULL)
    wrong_type_arg(proc, 2, s_right);

  CSGDifference *difference =
      new CSGDifference(solid_left, solid_right, material);
  return sceneobject2scm(difference);
}

SchemeObject *make_intersection(Scheme *scheme, SchemeObject *s_left,
                                SchemeObject *s_right,
                                SchemeObject *s_material) {
  const wchar_t *proc = L"make-intersection";
  Material *material;
  if (s_material == S_UNSPECIFIED) {
    material = NULL;
  } else {
  ...
  CSGIntersection *intersection =
      new CSGIntersection(solid_left, solid_right, material);
  return sceneobject2scm(intersection);
}

SchemeObject *make_julia(Scheme *scheme, SchemeObject *s_c,
                         SchemeObject *s_max_iter, SchemeObject *s_steps,
                         SchemeObject *s_accuracy, SchemeObject *s_w_offset,
                         SchemeObject *s_material) {
  const wchar_t *proc = L"make-julia";
...
SchemeObject *make_instance(Scheme *scheme, SchemeObject *s_object,
  ...
  if (obj == NULL) {
    ...
    if (dynamic_cast<ObjectGroup *>(sobj) != NULL) {
      cout << "Hint: If you're trying to make an transformed instance of an "
              "objectgroup, such as a mesh, try wrap it in a (make-bound ...) "
              "to make it intersectable."
           << endl;
      // TODO: Should we just do that bounding transparently to the user - for a
      // simpler experience?
    }
  }
...
SchemeObject *bounding_box(Scheme *scheme, SchemeObject *s_obj) {
  const wchar_t *proc = L"bounding-box";
...
SchemeObject *intersect(Scheme *scheme, SchemeObject *s_obj,
                        SchemeObject *s_origin, SchemeObject *s_direction) {
...
SchemeObject *all_intersections(Scheme *scheme, SchemeObject *s_obj,
                                SchemeObject *s_origin,
                                SchemeObject *s_direction) {
  ...
  Intersection *intersections =
      (Intersection *)::alloca(sizeof(Intersection) * obj->maxIntersections());
...
SchemeObject *inside_p(Scheme *scheme, SchemeObject *s_obj,
...
SchemeObject *
SceneObjectFactory::make_text(Scheme *scheme, SchemeObject *s_text,
                              SchemeObject *s_ttf_file, SchemeObject *s_size,
                              SchemeObject *s_depth, SchemeObject *s_material) {
  ...
  TrueTypeFont *font =
      new TrueTypeFont(SchemeFilenames::toFilename(wttf_filename));
...
void SceneObjectFactory::register_procs(Scheme *s) {
  ...
  scheme->assign(L"make-sphere", 2, 1, 0, (SchemeObject * (*)()) make_sphere);
  ...
  scheme->assign(L"make-box", 3, 0, 0, (SchemeObject * (*)()) make_box);
  ...
  scheme->assign(L"make-cone", 4, 1, 0, (SchemeObject * (*)()) make_cone);
  scheme->assign(L"make-uncapped-cone", 4, 1, 0,
                 (SchemeObject * (*)()) make_uncapped_cone);
  scheme->assign(L"make-torus", 2, 1, 0, (SchemeObject * (*)()) make_torus);
  ...
  scheme->assign(L"make-isosurface", 7, 0, 0,
                 (SchemeObject * (*)()) SceneObjectFactory::make_isosurface);
  scheme->assign(L"make-sdf-object", 3, 0, 0,
  ...
  scheme->assign(L"make-union", 2, 0, 1, (SchemeObject * (*)()) make_union);
  scheme->assign(L"make-parametrized-surface", 6, 0, 0,
                 (SchemeObject * (*)())
                     SceneObjectFactory::make_parametrized_surface);
  scheme->assign(L"make-julia", 6, 0, 0, (SchemeObject * (*)()) make_julia);
  scheme->assign(L"make-marching-cubes", 3, 0, 0,
                 (SchemeObject * (*)()) make_marching_cubes);
  scheme->assign(L"make-bound", 1, 0, 0, (SchemeObject * (*)()) make_bound);
  ...
  scheme->assign(L"$intersect", 3, 0, 0, (SchemeObject * (*)()) intersect);
  scheme->assign(L"$all-intersections", 3, 0, 0,
                 (SchemeObject * (*)()) all_intersections);
  scheme->assign(L"$inside?", 2, 0, 0, (SchemeObject * (*)()) inside_p);
}
