...
SchemeObject *iso_goursats_surface(Scheme *scheme, SchemeObject *s_a,
                                   SchemeObject *s_b, SchemeObject *s_c,
  ...
  double result =
      x2 * x2 + y2 * y2 + z2 * z2 + a * sumxyz * sumxyz + b * sumxyz + c;
  return double2scm(result);
}

SchemeObject *iso_torus(Scheme *scheme, SchemeObject *s_R, SchemeObject *s_r,
                        SchemeObject *s_x, SchemeObject *s_y,
                        SchemeObject *s_z) {
...
void SchemeFunctions::register_procs(Scheme *scheme) {
  ...
  scheme->assign(L"iso-goursats-surface", 6, 0, 0,
                 (SchemeObject * (*)()) iso_goursats_surface);
}
