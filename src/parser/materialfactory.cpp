...
SchemeObject *MaterialFactory::s_make_material(Scheme *scheme,
  ...
  for (uint32_t i = 0; i < argc; i++) {
    ...
    if (key == L"diffuse") {
      ...
      SchemeNormalPerturber *perturber =
          new SchemeNormalPerturber(scheme, s_value);
...
void MaterialFactory::register_procs(Scheme *s) {
  ...
  scheme->assign(L"make-material", 1, 0, 0,
                 (SchemeObject * (*)()) MaterialFactory::s_make_material);
}
