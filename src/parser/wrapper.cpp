...
struct wrapped_object *scm2wrappedobj(SchemeObject *s_smob, const wchar_t *subr,
                                      int pos) {
  assert_arg_wrapped_type(subr, pos, s_smob, wrapped_object_tag) return (
      struct wrapped_object *)s_smob->getWrappedCObject();
}
...
SchemeObject *isWrappedObjectType(SchemeObject *object, wrapped_type type) {
  if (isWrappedObject(object)) {
    struct wrapped_object *o =
        (struct wrapped_object *)object->getWrappedCObject();
...
SceneObject *scm2sceneobject(SchemeObject *object_smob, const wchar_t *subr,
                             int pos) {
  struct wrapped_object *o = scm2wrappedobj(object_smob, subr, pos);
...
SamplerFactory *scm2sampler(SchemeObject *object_smob, const wchar_t *subr,
                            int pos) {
  struct wrapped_object *o = scm2wrappedobj(object_smob, subr, pos);
  if (o->type != SAMPLER) {
    wrong_type_arg(subr, pos, object_smob);
...
Material *scm2material(SchemeObject *object_smob, const wchar_t *subr,
                       int pos) {
  struct wrapped_object *o = scm2wrappedobj(object_smob, subr, pos);
  if (o->type != MATERIAL) {
    wrong_type_arg(subr, pos, object_smob);
...
Lightsource *scm2lightsource(SchemeObject *object_smob, const wchar_t *subr,
                             int pos) {
  struct wrapped_object *o = scm2wrappedobj(object_smob, subr, pos);
