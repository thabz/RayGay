...
CatmullRomSpline::~CatmullRomSpline() { P.clear(); }
...
Vector CatmullRomSpline::getPoint(double t) const {
  ...
  double tt = t * t;
  double ttt = tt * t;
  return 0.5 * ((-ttt + 2 * tt - t) * P[i - 1] + (3 * ttt - 5 * tt + 2) * P[i] +
                (-3 * ttt + 4 * tt + t) * P[i + 1] + (ttt - tt) * P[i + 2]);
}
...
Vector CatmullRomSpline::getTangent(double t) const {
  ...
  double tt = t * t;
  Vector res;
  res = 0.5 * ((-3 * tt + 4 * t - 1) * P[i - 1] + (9 * tt - 10 * t) * P[i] +
               (-9 * tt + 8 * t + 1) * P[i + 1] + (3 * tt - 2 * t) * P[i + 2]);
  res.normalize();
  return res;
}
