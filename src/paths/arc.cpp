 ...
 */
Arc::Arc(const Vector &center, double radius, const Vector &normal,
         double begin_degree, double end_degree) {
...
Vector Arc::getPoint(double t) const { return circle->getPoint(convert(t)); }

Vector Arc::getTangent(double t) const {
  return circle->getTangent(convert(t));
}

void Arc::transform(const Matrix &m) { circle->transform(m); }
