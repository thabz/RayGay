...
void Stats::dump() const {
    ...
    for(cur_time = beginTimes.begin(); cur_time != beginTimes.end(); cur_time++) {
	    ...
	    if (endTimes.find(cur_time->first) != endTimes.end()) {
	        int64_t secs = int64_t(double(endTimes.find(cur_time->first)->second - cur_time->second) / CLOCKS_PER_SEC);
	        cout << ": ";
...
void WalltimeStats::out() const 
{
    ...
    secs += double(end_time.tv_nsec - begin_time.tv_nsec) / 1000000000.0;
    int whole_hours = int(secs / 3600);
    int whole_minutes = int(secs / 60);
    int whole_seconds = int(secs);
    int hundreds = int(100.0 * (secs - whole_seconds));
